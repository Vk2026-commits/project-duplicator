...
BEGIN
  ...
  UPDATE public.startups SET invested = _total WHERE id = _startup_id;
  ...
  IF TG_OP = 'UPDATE' AND OLD.startup_id IS DISTINCT FROM NEW.startup_id THEN
    ...
    UPDATE public.startups SET invested = _total WHERE id = OLD.startup_id;
  END IF;
...
CREATE TRIGGER trg_sync_startup_invested
AFTER INSERT OR UPDATE OR DELETE ON public.startup_investors
FOR EACH ROW EXECUTE FUNCTION public.sync_startup_invested();
